...
class AnymalEnv(DFlexEnv):
    ...
    def __init__(self, device='cuda:0', num_envs=4096, seed=0, episode_length=1000, img_height=84, img_width=84, no_grad=True, stochastic_init=False, MM_caching_frequency=1, early_termination=True):
        ...
        super(AnymalEnv, self).__init__(num_envs, num_state_obs, num_act, episode_length, MM_caching_frequency, seed, 
        ...
        self.renderer = ManiskillViewer(env_name="AnymalVis", num_env=num_envs, height=img_height, width=img_width)
    ...
    def init_sim(self):
        ...
        for i in range(self.num_environments):
            lu.urdf_load(
                self.builder, 
                os.path.join(asset_folder, 'anymal_c/urdf/anymal_minimal.urdf'),
                df.transform((0.0, 0.55, 0.0), df.quat_from_axis_angle((1.0, 0.0, 0.0), -math.pi*0.5)), 
                floating=True,
                stiffness=85.0,
                damping=2.0,
                shape_ke=2.0e3,
    ...
    def calculateStateObservations(self):
        ...
        self.state_obs_buf = torch.cat([torso_pos[:, 1:2], # 0
                                ...
                                lin_vel, # 5:8
                                ang_vel, # 8:11
                                self.state.joint_q.view(self.num_envs, -1)[:, 7:], # 11:23
                                self.joint_vel_obs_scaling * self.state.joint_qd.view(self.num_envs, -1)[:, 6:], # 23:35
                                up_vec[:, 1:2], # 35
                                (heading_vec * target_dirs).sum(dim = -1).unsqueeze(-1), # 36
                                self.actions.clone()], # 37:49
                                dim = -1)
    """
    ...
    def calculateReward(self):
        up_reward = self.up_rew_scale * self.state_obs_buf[:, 35]
        heading_reward = self.heading_rew_scale * self.state_obs_buf[:, 36]
        height_reward = self.heigh_rew_scale * (self.state_obs_buf[:, 0] - self.termination_height)
